...
public class ManagerServlet extends HttpServlet {
	...
	private void login(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (admin.getUsername() != null && admin.getPassword() != "") {
			req.setAttribute("message", "登陆成功");
			session.setAttribute("admin", admin);
			req.getRequestDispatcher("/manager/message.jsp").forward(req, resp);
		} else {
			resp.getWriter()
					.write("登录错误，请重新登录！！      <a href='http://localhost:8080/xinhua_library/index.jsp'>返回首页</a>");
		}
	...
	private void layout(HttpServletRequest req, HttpServletResponse resp) {
		try {
			...
			resp.sendRedirect("http://localhost:8080/xinhua_library/manager/managerLogin.jsp");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
