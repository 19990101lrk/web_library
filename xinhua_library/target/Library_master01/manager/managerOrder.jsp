...
<body>
	...
	<script type="text/javascript">
		...
		function delOne(ordernum) {
			var sure = window.confirm("确定删除改选项吗？");
			if (sure) {
				window.location.href = "${pageContext.request.contextPath}/order/OrderServlet?op=delOrder&ordernum="
						+ ordernum;
				alert("删除成功！！");
			}
		}
		...
		<c:forEach items="${orders}" var="os" varStatus="vs">
			<tr class="active">
				...
				<c:if test="${os.status==1}">
					<td>已发货</td>
<%--					<td><a href="javascript:delOne('${os.ordernum}')">删除</a></td>--%>
					<td><a href="#">删除</a></td>
				</c:if>
			</tr>
		</c:forEach>
